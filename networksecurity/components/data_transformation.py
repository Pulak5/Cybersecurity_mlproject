...
class DataTransformation:
    def __init__(self,data_transformation_config:DataTransformationConfig,data_validation_artifact:DataValidationArtifacts):
        try:
    ...
    def initiate_data_transformation(self)->DataTransformationArtifacts:
        logging.info("Entered into the initiate_data_transformation method in DataTransformation")
        try:
            logging.info("Starting data transformation")
            train_set=self.read_data(self.data_validation_artifact.valid_train_file_path)
            test_set=self.read_data(self.data_validation_artifact.valid_test_file_path)
            
            input_feature_train_df=train_set.drop(columns=[TARGET_COLUMN])
            target_feature_train_df=train_set[TARGET_COLUMN]
            target_feature_train_df=target_feature_train_df.replace(-1,0)
            input_feature_test_df=test_set.drop(columns=[TARGET_COLUMN])
